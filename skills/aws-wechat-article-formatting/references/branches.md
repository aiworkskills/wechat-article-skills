...
曾有一条内置兜底，套主题里的 `highlight` 样式键。**没有一套模版定义过 `highlight`**，兜底一路退回 `blockquote`，渲出来和普通引用块逐字相同——两个语义不同的东西长成一样，正是给导语单独做样式时要避开的问题。加上 `:::` 写手已经不产出，留着就是养一个假功能。

现在 `:::highlight` 走上面的「未知组件」路径：告警 + 按原文输出，不吞内容。**真要提示框**，放一个 `highlight.yaml` 组件文件（内置或用户目录都行），走通用组件路径即可。
