...
### 满版 / 冲出边距 ⛔ 做不了（探针 2 · 2026-09-07 手机实测）

三种绕开 calc 的写法全部失败，元素都停在文字列宽内：

    margin:0 -20px + padding:20px + box-sizing:content-box   ✗
    margin:0 -20px 不给 width（等 auto 拉伸）                  ✗
    图片当 section 背景 + 负 margin                           ✗
    img width:120%; max-width:120%                           ✗  被微信注入的 max-width:100% 压回

结论：**微信正文有约 20px 的固定边距，块级元素和图片都被限制在这个列宽内，
无法贴到屏幕两边。** calc 被删、vw 被 max-width 压回、负 margin 只平移不拉伸。
杂志风参照图里的边到边照片在公众号里**不可复制**，设计时以 ≈335px 列宽为满宽。

能做的只有**单侧微出血**：探针 1 里 `margin-left:-16px` 能把元素拉到距屏约 4px
（截图实测），做「挂在左边」的构图可以用，但到不了边。

### 带引号的字体名可用

`font-family:'Bodoni 72'` 回读后引号编码成 `&#39;`，手机上正确渲染为高对比衬线。
含数字的字体名**必须**加引号（CSS 语法要求），微信不会因此删声明。
（此前文档里「url() 里引号会被过滤」那条针对的是 `background:url()`，与 font-family 无关。）

### 对四套模版意味着什么

杂志风参照图里的手法，**除了「文字绕图」「灰度/模糊滤镜」「满版贴边」三样，其余全部可用**。
