...
- **shell**：仅 `{python} {baseDir}/scripts/product_image_ingest.py`、`import_presets_aws.py`（`{python}` = 本机 Python 3 解释器，见 [首次引导「检测顺序」](../aws-wechat-article-main/references/first-time-setup.md)：Windows 用 `py -3 -X utf8`，macOS / Linux 用 `python3`）
