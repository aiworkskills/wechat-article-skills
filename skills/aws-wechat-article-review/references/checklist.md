...
- [ ] 脚本退出码为 0（硬性项 → 🔴，提醒项 → 🟡）
- [ ] **读一遍它打印的加粗串**：像不像一份能独立看懂的提要？像词云或像把段落重念一遍，都是挑错了——这一条脚本量不出来

覆盖的是产出配额里能数出来的部分：`>` 导语、加粗密度与长度、关键数字覆盖、`- **标签**：说明` 列表、带出处的金句恰好一处。与 [writing 第 7 步](../../aws-wechat-article-writing/SKILL.md) 同一套判据。
...
- [ ] 与正文内容一致
- [ ] 不是正文第一段的复制
- [ ] **与正文首个 `>` 导语不是同一句话**（实测 7 篇逐字相同——摘要在后台列表里出现一次、导语在正文开头再出现一次，读者等于同一段读两遍）
...
- [ ] `imgs/` 目录存在
- [ ] 封面图已生成，在**文章目录下**（`cover.png` / `.jpg` / `.jpeg` / `.webp`）——**不在 `imgs/` 里**，`publish.py upload-thumb` 也是按这个位置找
