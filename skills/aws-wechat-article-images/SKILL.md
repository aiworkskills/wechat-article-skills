...
- **全局**：读 **`.aws-article/config.yaml`** — `cover_aspect`、`cover_style`、`image_density`、`caption_style`、**`custom_cover_image_style`** > **`default_cover_image_style`**、**`custom_article_image_style`** > **`default_article_image_style`**（**须为 YAML 字符串列表**；`custom_*` 非空时优先于 `default_*`；多候选时须按主题在本篇 **`article.yaml`** 同键改为**单元素列表**后再加载 **`presets/image-styles/<名>.md`**）、`multi_image_count`、`tone` 等以之为准（完整字段见 [articlescreening-schema.md](../aws-wechat-article-main/references/articlescreening-schema.md) 与 **`config.example.yaml`**）。
...
- **封面图**：`custom_cover_image_style`（优先）> `default_cover_image_style` 解析出**单一**预设名则用之；未配置或空列表则直接进入下列优先级链。
- **正文配图**：`custom_article_image_style`（优先）> `default_article_image_style` 同上。
