...
**先跑一次**，把候选池里认不出的形态名挑出来（不调 API）：

```bash
{python} {baseDir}/scripts/image_create.py styles
```

退出码 1 说明 `config.yaml` 的候选池里有不存在的名字——**先改名再往下走**。实测网站导出的 config 写过「对比说明 / 板书白板 / 氛围烘托」，三个都没有对应模板（真名是 `对比两栏` / `概念隐喻` / `场景还原`，而「板书白板」是**媒介**不是形态）。挑中这种名字只能让模型自由发挥，整套形态方法论静默失效。
