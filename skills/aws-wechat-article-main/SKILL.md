...
3. **建目录与 `article.yaml`**：创建 `{drafts_root}/YYYYMMDD-标题slug/`（`drafts_root` 以 **`config.yaml`** 为准，默认 `drafts/`）。随即初始化本篇 **`article.yaml`**（含 **`publish_completed: false`**，及标题、作者、摘要等；目录内**最先写入**）。
   - ⛔ **必须用 `{baseDir}/../aws-wechat-article-publish/scripts/article_init.py` 初始化，不要手写。** 手写一定会漏字段——实测三篇连着漏掉 `image_medium`，配图媒介的「避开上一篇」因此永远查不到东西，整个号的配图退化成随机。
   - **`publish_method`**：默认 **`draft`**；要发出去 → **`published`** 或 **`full --publish`**；用户明确不填微信 → **`none`**。
4. **本篇预设单选落盘（必做）**：初始化后按场景执行：**新建首轮**以 **`.aws-article/config.yaml`** 为来源，按 **`custom_* > default_*`** 结合本篇主题/选题卡，为以下字段各选**单一预设**并写回本篇 **`article.yaml`** 为**单元素列表**：`default_structure`、`default_closing_block`、`default_title_style`、`default_format_preset`、`default_format_scheme`、`default_cover_image_style`、`default_sticker_style`。

   - ⛔ **`default_article_image_style` 不在此列，保持多元素候选池原样。** 正文配图形态是**每个图位各选一个**（这一节讲流程用流程图，那一节讲数字用数据图表），由 [image-method 第二步](../aws-wechat-article-images/references/image-method.md) 逐位判断。收敛成一个等于让整篇所有配图都用同一种形态——实测三篇都被钉成「对比说明」，而实际三张图分别是实证、流程图、流程图，没有一张对得上。
   - ⛔ **选排版模版与配色时不要只看名字**：先跑 **`format.py --list-themes`**，按其中「适合 / 不适合」对本篇题材，再按配色说明选一档。**也不要连着几篇都选同一套。** 开工前先扫一眼最近几篇的 `article.yaml`，模版和配色都与上一篇不同——同一天发的几篇如果模版、配色、封面形态四个维度全同，读者一眼看出是同一个模板套出来的。
