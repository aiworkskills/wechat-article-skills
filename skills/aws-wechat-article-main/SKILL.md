...
⛔ 输出为 `missing`（任一文件不存在）→ 按 [首次引导](references/first-time-setup.md) 创建或补全：可参考 **`skills/aws-wechat-article-main/references/config.example.yaml`** 得到 **`config.yaml`**，在仓库根创建 **`aws.env`**（仅密钥与微信 `WECHAT_N_APPID` / `WECHAT_N_APPSECRET` 等，键名可与 `skills/aws-wechat-article-main/references/env.example.yaml` 对照）。
...
- **`aws.env`**（仓库根，密钥）与 **`.aws-article/config.yaml`**（非密钥与模型/微信元数据及 **`publish_method`**）：校验 **`skills/aws-wechat-article-main/scripts/validate_env.py`**；`aws.env` 键名见 **`references/env.example.yaml`**
