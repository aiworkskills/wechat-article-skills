...
**只保留下游真会用到的四个。** 此前有六个（多「流程图」「对比」），但它们和「信息图」在下游没有任何差别——配图阶段会按内容重新判形态（流程步骤 / 对比两栏 / 数据图表…），标记里的细分只是让写手多猜一次。

| 类型名 | 下游拿它做什么 | 示例 |
|--------|--------------|------|
| `封面` | 排版跳过它不进正文；发布时拿它当文章封面 | `![封面：透明水杯与小闹钟，宽幅留白适合标题](placeholder)` |
| `实证` | **必须是真实素材**（官方原帖截图、界面实拍），不许生成；说明文件放 `imgs/sources/` | `![实证：Cognition 发布 SWE-2 的原帖](placeholder)` |
| `信息图` | 信息位——解释内容用，流程/结构/数据/对比都算；`caption_style: 关键图有` 时只有它和实证会出图注 | `![信息图：5个工具的功能对比，列=名称/价格/特点](placeholder)` |
...
> **注意**：上表是结尾的**写法/语气**，不是要照搬的小标题。互动型收尾必须**直接写成一句自然的话**（一个抛给读者的开放式问题即可），**严禁**在它前面加「互动话题」「互动时间」「话题互动」「互动引导」之类的标题或加粗标签——那是典型的 AI 味露馅。
