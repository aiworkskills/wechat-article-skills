...
`default_structure` / `default_closing_block`（及对应的 `custom_structure` / `custom_closing_block` 覆盖层）指向的 **预设正文**仍来自 **`.aws-article/presets/`**（及用户目录下同名 presets），与配置中的**文件主名**对应。二者**须为 YAML 列表**：`[]`、`[名]`，或多项候选；**多项时**须先在本篇 **`article.yaml`** 同键改为**单元素列表**再调用 **`write.py`**（勿用字符串标量）。`custom_*` 非空时优先于 `default_*`。
...
1. 用户指定（「用清单体结构」）
2. 合并约束中的 `custom_structure` > `default_structure`（若有；多候选列表须已在本篇 **`article.yaml`** 收敛为单项，见 schema）
...
2. `.aws-article/presets/closing-blocks/` 下 `custom_closing_block` > `default_closing_block` 指定的文件
