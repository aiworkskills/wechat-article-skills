...
- **shell**：仅 `{python} {baseDir}/scripts/write.py`（`{python}` = 本机 Python 3 解释器，见 [首次引导「检测顺序」](../aws-wechat-article-main/references/first-time-setup.md)：Windows 用 `py -3 -X utf8`，macOS / Linux 用 `python3`）
...
**产出配额自检 ⛔**：`draft.md` 写完后**跑一遍脚本**，不合格先补再往下走。这些实测最容易漏，而且漏了不报错、只是版式悄悄少一块：

```bash
{python} {baseDir}/scripts/write.py check drafts/YYYYMMDD-slug/draft.md
```

它会把**全文的加粗抽出来串成一行**打印。加粗的用途是让读者从密密麻麻的正文里一眼抓住关键信息，所以验收标准就是**这一串读起来像不像一份提要**——串不起来说明挑错了，不是数量不够。退出码非 0 表示有硬性项没达标。

下面这张表是同一套判据的说明，脚本报了哪条就回来看哪条：
