...
> **Optional add-on · cue-omni-reader (not part of this kit)**: if you also need to turn PDFs / authorized local documents, audio, or video / other web pages into source text for writing, you can install the external skill **cue-omni-reader** ([`sensedeal/cue-skills`](https://github.com/sensedeal/cue-skills) → [`cue-omni-reader`](https://github.com/sensedeal/cue-skills/tree/main/cue-omni-reader), MIT). It is an instruction-only layer that drives the official Cue Omni Reader MCP to parse an HTTP(S) URL or an authorized local document, audio, or video source into Markdown. Install: `npx skills add sensedeal/cue-skills --skill cue-omni-reader`. This kit does not bundle it; follow the upstream README for install and usage.
