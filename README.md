...
| **排版** | Markdown → 微信 HTML，8 套模版 × 3 套配色 + 9 个版式组件 | 「排版」 |
...
> ```
>
> 内置 9 个：导语 / 编号小标题 / 金句卡 / 数据卡 / 步骤流程 / 对比两栏 / 结构分层 / 要点清单 / 文末区块。模板里的颜色从当前配色取值，和任意模版 × 配色组合都不脱节；自定义放 `.aws-article/presets/components/`，同名覆盖。每个组件的 YAML 都带 `when_to_use` / `when_not_to_use` / `anti_pattern`——和配图方法里的判据同一个作用：拦住「因为好看所以用」。
>
...
<details open>
<summary><b>排版模版</b> — 排版分「模版 × 配色」两层：8 套模版各 3 套配色。skill 内置 4 套模版兜底（亲和 / 资讯 / 书卷 / 杂志），另 4 套（活力 / 手账 / 硬朗 / 技术）随 <code>.aws</code> 预设包下发，也能 YAML 自定义</summary>
