...
## [2026-09-06] — 版式组件层：主题之外能表达结构

### 新增

- **版式组件（`:::` 块）**：主题只能给标签配内联样式，而微信正文**没有伪元素**——「小标题前的编号角标」「引用块的大引号」这类装饰没法靠 CSS 变出来，必须真的插元素，所以再怎么调主题，天花板都是改颜色和间距。组件补的就是这一层：
  ...
  内置 7 个：`section-title`（编号小标题）、`quote-card`（金句卡）、`stat`（数据卡）、`steps`（步骤流程）、`compare`（对比两栏）、`layers`（结构分层）、`checklist`（要点清单）。定义为 YAML（HTML 模板写在里边），内置在 `references/components/`，自定义放 `.aws-article/presets/components/`，同名覆盖
- 组件模板里的 `{primary-color}` / `{text-color}` 等占位符从当前主题取值，组件与任何主题组合都不脱节；未知组件名或缺少结尾 `:::` 时按原文输出并告警，不吞内容；方括号参数做 HTML 转义
- 每个组件带 **`when_to_use` / `when_not_to_use` / `anti_pattern`**，是给 Agent 选型用的契约，与配图方法里的判据同一个作用：拦住「因为好看所以用」
- **配图与组件的分工判据**（`image-method.md` 第一步半）：真写一篇后发现同一组数字出现了三次（数据卡 + 对比配图 + 对比组件）——两套机制干同一件事，不分工必然重复。判据不是「是不是文字」而是**有没有空间关系**：有大小 / 流向 / 嵌套 → 用图，纯文字并列 / 对照 / 枚举 → 用组件。五个形态逐一做了同内容对照：数据图表 / 流程步骤 / 结构分层归**图**，对比两栏 / 清单要点归**组件**（「数字 → 组件」这个先验被推翻了——数据图表全是数字，却是图赢得最彻底的一项）
- **3 套内置主题**：克制（量子位 / 新智元 那一路，零装饰件）、工程笔记（小标题带 `#` 前缀与等宽字）、杂志（衬线大字、细线分节），内置主题增至 **7 套**
- `references/wechat-html-constraints.md`：微信正文的 HTML / CSS 约束——只认内联样式，没有伪元素与伪类，`position` 与 `id` 会被整条删掉。实测推翻了传闻：走 API 发布时微信几乎不过滤

### 变更

- **图注只认显式写的 `title`**：`![信息图：画面指令](imgs/x.png "图注给读者看")`。此前 `format.py` 直接拿 alt 冒号后那段当图注，而那段是给生图模型的画面 brief——产出过「开发者站在巨型 99.9 分数牌前，视线越过分数望向复杂而开放的城市与工作现场」这种图注，把读者眼睛已经看见的复述一遍，零信息；图没生成出来时同一句话会出现两次。**没写 title 就不出图注**，这是有意的：绝大多数图不需要图注，错的图注比没有更糟。写作提示词同步改，否则模型不知道有第三个参数

### 修复

- **16 套网站主题的组件全是兜底蓝**：组件模板用 `{primary-color}` 取色，但网站导出的主题只有字面色的 `styles`、不带 `variables` 块，于是 16 套全部落到 `DEFAULT_VARIABLES` 的 `#0F4C81`，小标题一个色。主题没写 `variables` 时改为从它自己的样式反推（`strong` → `a` → `h3` → `h2` 取第一个 color），显式 `variables` 与 `--color` 仍优先。取 `strong` 比取 `primaryColor` 更贴实际观感：故宫黄的 `#dda52d` 在白底上偏浅，`strong` 用的 `#9C6A15` 才是可读的那个
- `checklist` 的状态列直接把 `done` 打进 18px 宽的格子会渲染成截断的 `don`；新增 `row_map` 支持值映射成符号，写错的值原样输出、不让整行消失
